...
class IngredientForm(FlaskForm):
    ...
    measurement_type = SelectField(
        'Measurement Type', choices=[('grams', 'grams'), ('milliliters', 'milliliters'), ('pieces', 'pieces')],
...
class RecipeForm(FlaskForm):
    ...
    description = TextAreaField(
        'Description', validators=[DataRequired()], render_kw={"class": "materialize-textarea"}
    )
    instructions = TextAreaField(
        'Instructions', validators=[DataRequired()], render_kw={"class": "materialize-textarea"}
    )
...
@login_required
def view_recipes():
    page = request.args.get('page', 10, type=int)  # Get the page number from the query string
    search_term = request.args.get('search', '')

    if search_term:
        recipes = models.Recipe.query.join(models.Ingredient).filter(
    ...
    return render_template(
        'recipes.html', recipes=recipes.items, pagination=recipes,
        breadcrumbs=breadcrumbs, search_term=search_term
    )
...
def add_recipe():
    ...
    only_csrf_errors = all(len(e) == 1 and 'csrf_token' in e for e in form.errors.get('ingredients', []))
    ...
    return render_template('recipe_form.html', form=form, form_metadata=form_metadata, breadcrumbs=breadcrumbs)
...
def edit_recipe(id):
    ...
    if request.method == "POST" and (form.validate_on_submit() or all(
    ...
    return render_template(
        'recipe_form.html', form=form, recipe=recipe, form_metadata=form_metadata,
...
def view_recipe(id):
    ...
    if recipe.author != current_user:
        ...
        return redirect(url_for('recipe.view_recipes'))
    return render_template(
        'recipe_view.html', recipe=recipe, breadcrumbs=breadcrumbs
    )
